...
public interface BlockWorker extends Worker, SessionCleanable {
  ...
  /**
   * Creates a block. This method is only called from a data server. Calls
   * {@link #getTempBlockWriterRemote(long, long)} to get a writer for writing to the block. Throws
   * an {@link IllegalArgumentException} if the location does not belong to tiered storage.
   *
   * @param sessionId the id of the client
  ...
  /**
   * Similar as {@link BlockWorker#getStoreMeta} except that this also contains full blockId list.
  ...
  /**
   * Closes a UFS block for a client session. It also commits the block to Alluxio block store if
   * the UFS block has been cached successfully.
   *
   * @param sessionId the session ID
   * @param blockId the block ID
   ...
   */
  void closeUfsBlock(long sessionId, long blockId) throws BlockAlreadyExistsException,
   ...
   */
  void deleteWorker(long sessionId, String availableWorker, long transferByte) throws IOException;
