...
final class PipelineHandler extends ChannelInitializer<Channel> {
  ...
  public PipelineHandler(WorkerProcess workerProcess) {
    ...
    mFileTransferType = Configuration.getEnum(PropertyKey.WORKER_NETWORK_NETTY_FILE_TRANSFER_TYPE,
  ...
  protected void initChannel(Channel ch) throws Exception {
    ...
    pipeline.addLast("idleEventHandler",
        new IdleStateHandler(timeoutMs, 0, 0, TimeUnit.MILLISECONDS));
    ...
    pipeline.addLast("dataServerShortCircuitReadHandler", new DataServerShortCircuitReadHandler(
        NettyExecutors.RPC_EXECUTOR, mWorkerProcess.getWorker(BlockWorker.class)));
    ...
    pipeline.addLast("dataServerUnsupportedMessageHandler",
        new DataServerUnsupportedMessageHandler());
  }
}
